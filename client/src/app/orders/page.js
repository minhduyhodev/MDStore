...
const STATUS_DISPLAY = {
  COMPLETED:            { label: 'Hoàn thành',   icon: CheckCircle2, className: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/20' },
  PENDING:              { label: 'Đang xử lý',   icon: Clock,        className: 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-500/20' },
  FAILED_PRICE_CHANGED: { label: 'Lỗi giá đổi',  icon: AlertCircle,  className: 'bg-rose-500/10 text-rose-600 dark:text-rose-400 border-rose-500/20' },
  PROCESSING_RETRY:     { label: 'Đang thử lại', icon: RefreshCw,    className: 'bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/20' },
};
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
              <h4 className="text-slate-500 dark:text-slate-400 font-medium mb-1">Đơn thành công</h4>
              <div className="text-4xl font-black text-slate-800 dark:text-slate-100 mb-4">2</div>
              <div className="text-sm text-emerald-500 flex items-center gap-1 font-medium">Tỷ lệ hoàn thành 40%</div>
            </div>
            
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
              <h4 className="text-slate-500 dark:text-slate-400 font-medium mb-1">Cần xử lý</h4>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-end sm:items-center gap-4">
            <h2 className="text-xl font-bold text-slate-800 dark:text-white">Lịch sử giao dịch</h2>
            <div className="flex gap-3">
              <button className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:bg-slate-50 text-sm font-medium transition-colors">
                <Filter size={16} /> Lọc
              </button>
              <button className="flex items-center gap-2 px-4 py-2 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-500/20 rounded-xl hover:bg-indigo-100 text-sm font-medium transition-colors">
          ...
          {/* Table */}
          {loading ? (
             <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 h-96 animate-pulse"></div>
          ) : (
            <motion.div 
                    ...
                    {orders.map((order) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="py-4 px-6">
                            <div className="font-semibold text-slate-800 dark:text-slate-200">{order.productName}</div>
                          </td>
